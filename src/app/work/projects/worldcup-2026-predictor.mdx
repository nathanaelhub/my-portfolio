...
- **Upcoming fixtures** — the next real matches (remaining group games, or the Round of 32 once the groups finish), each with a most-likely scoreline and an outcome-probability bar.
- **Bracket** — a predicted path to the final: the model advances the favourite through every knockout tie and crowns a champion, flagging upsets where the lower-rated side wins. The 32-team field comes from the real qualifiers (or a projected table while the groups are still in play).
