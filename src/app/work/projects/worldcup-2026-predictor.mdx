...
- **Upcoming fixtures** — only matches still to be played, each with a most-likely scoreline and an outcome-probability bar. During the groups it shows the remaining group games; once they finish it shows the real Round-of-32 draw (official pairings), with results auto-filling from the dataset so only predictions *ahead* remain.
